...
/** Minimal Logging stub for the vendored ClosureCleaner.
  ...
  * `logTrace`/`logDebug`/`logInfo`/`logWarning`/`logError` and a `log` value with `isDebugEnabled`
  * / `isTraceEnabled`. SC3 doesn't depend on Spark, so we provide a no-op shim with the same
  * surface. This keeps the vendored ClosureCleaner source unchanged.
  *
...
trait Logging:
  ...
  protected def logTrace(msg: => String, throwable: Throwable): Unit =
    if log.isTraceEnabled then
      Log.print("TRACE", s"$msg ${throwable.getClass.getName}: ${throwable.getMessage}")
  ...
  protected def logDebug(msg: => String, throwable: Throwable): Unit =
    if log.isDebugEnabled then
      Log.print("DEBUG", s"$msg ${throwable.getClass.getName}: ${throwable.getMessage}")
