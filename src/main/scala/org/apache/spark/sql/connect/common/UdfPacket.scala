...
/** A serializable wrapper for a UDF function and its encoder metadata.
  ...
  * instances use `writeReplace` to substitute an [[EncoderSerializationProxy]] that, on the
  * server side, resolves to the server's own encoder singletons via reflection. This avoids
